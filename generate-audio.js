...
const API_KEY = process.env.ELEVENLABS_API_KEY ||
  fs.readFileSync(path.join(__dirname, '../../api-keys.env'), 'utf8')
    .match(/ELEVENLABS_API_KEY=(.+)/)?.[1]?.trim();

if (!API_KEY) {
  console.error('ERROR: No ElevenLabs API key found.');
  console.error('Set ELEVENLABS_API_KEY env var or add it to ~/api-keys.env');
  process.exit(1);
}
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════╗');
  console.log('║   GetDeaddicted Academy — ElevenLabs Audio Generator ║');
  console.log('╚══════════════════════════════════════════════════════╝\n');
  ...
  const courses = singleCourse
    ? COURSES.filter(c => c.id === singleCourse)
    : COURSES;

  if (courses.length === 0) {
    console.error(`Course ${singleCourse} not found.`);
    process.exit(1);
  }
    ...
    for (let i = 0; i < narrations.length; i++) {
      ...
      if (fs.existsSync(outputPath)) {
        ...
        if (stat.size > 1000) { // Valid MP3 > 1KB
          skipped++;
          process.stdout.write(`   ⏭ Slide ${i} [${n.slideType}] already exists\n`);
          continue;
        }
      }

      process.stdout.write(`   🔊 Slide ${i}/${narrations.length - 1} [${n.slideType}] (${n.text.length} chars)... `);

      try {
        const size = await generateAudio(n.text, outputPath);
        completed++;
  ...
  // Save manifest for the player
  const manifest = {};
  for (const { course, narrations } of courseNarrations) {
    manifest[course.id] = narrations.map((n, i) => ({
      slide: i,
      type: n.slideType,
      module: n.module || null,
      file: `course-${course.id}/slide-${i}.mp3`,
      chars: n.text.length
    }));
  }
  fs.writeFileSync(
